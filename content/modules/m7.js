export const m7 = {
  ...
  etapes: [
    ...
    // ─── Étape 6 (Capstone) ──────────────────────────────────────────────────────
    {
      ...
      decouverte: {
        fr: "Une application web n'est pas la somme de morceaux indépendants — c'est un tout où le HTML définit les éléments que le CSS habille et que le JavaScript anime. Assembler le projet complet dans tes fichiers <code>app_web_meteo/</code> est le meilleur moyen de voir cette synergie en action.",
        en: "A web application is not just a sum of independent pieces — it is a whole where HTML defines the elements that CSS dresses and JavaScript animates. Assembling the complete project in your <code>app_web_meteo/</code> files is the best way to see this synergy in action."
      },
      explication: {
      ...
      exercice: {
        enonce: {
          fr: "Assemble les trois fichiers complets dans ton dossier <code>app_web_meteo/</code> avec Notepad++. Le HTML et le CSS sont déjà complets — complète les lignes marquées <code>// À compléter</code> dans le JS pour que l'application soit entièrement fonctionnelle. Tu peux tester ici, puis copier ta correction dans Notepad++.",
          en: "Assemble the three complete files in your <code>app_web_meteo/</code> folder using Notepad++. The HTML and CSS are already complete — fill in the lines marked <code>// À compléter</code> in the JS to make the application fully functional. You can test here, then copy your correction into Notepad++."
        },
        fichiers: {
      ...
      application: {
        fr: "🎉 <strong>Bravo, ton projet est complet !</strong> Tu viens d'assembler une vraie application web de A à Z : structure HTML, style CSS responsive, appels APIs, persistance, heure locale et gestion d'erreurs. Enregistre les trois fichiers dans ton dossier <code>app_web_meteo/</code>, ouvre <code>index.html</code> dans le navigateur et ajoute quelques villes — tu verras TON tableau de bord fonctionner pour de vrai. Le Module 8 propose des <strong>défis optionnels</strong> pour aller encore plus loin (mode sombre, export, météo étendue…) — à toi de jouer !",
        en: "🎉 <strong>Congratulations, your project is complete!</strong> You have just assembled a real web application from scratch: HTML structure, responsive CSS styling, API calls, persistence, local time, and error handling. Save all three files in your <code>app_web_meteo/</code> folder, open <code>index.html</code> in the browser, and add a few cities — you will see YOUR dashboard working for real. Module 8 offers <strong>optional challenges</strong> to go even further (dark mode, export, extended weather…) — it is your turn!"
      },
      quiz: [
        {
          type: "vraifaux",
