export const m6 = {
  ...
  etapes: [
    ...
    // ─── Étape 4 ────────────────────────────────────────────────────────────────
    {
      ...
      application: {
        fr: "Les cartes affichent maintenant la <strong>vraie température</strong> récupérée en temps réel. Les erreurs (ville introuvable, réseau indisponible) sont gérées proprement sans planter la page. Le prochain module ajoute l'heure locale et la persistance des villes.",
        en: "The cards now display the <strong>real temperature</strong> fetched live. Errors (city not found, network unavailable) are handled cleanly without crashing the page. The next module adds local time and city persistence."
      },
      notepad: {
        fr: {
          fichier: "script.js",
          intro: "Remplace tout <code>script.js</code> par ce code avec <code>async/await</code>, les deux requêtes et la gestion d'erreurs :",
          code: "const formulaire = document.querySelector(\"#form-ville\");\nconst champ = document.querySelector(\"#champ-ville\");\nconst conteneur = document.querySelector(\"#cartes\");\n\nformulaire.addEventListener(\"submit\", async function(event) {\n  event.preventDefault();\n  const ville = champ.value;\n  if (ville === \"\") return;\n\n  try {\n    const urlGeo = \"https://geocoding-api.open-meteo.com/v1/search?name=\" + ville + \"&count=1&language=fr\";\n    const repGeo = await fetch(urlGeo);\n    const geo = await repGeo.json();\n\n    if (!geo.results) { alert(\"Ville introuvable : \" + ville); return; }\n\n    const res = geo.results[0];\n    const urlMeteo = \"https://api.open-meteo.com/v1/forecast?latitude=\" + res.latitude\n      + \"&longitude=\" + res.longitude\n      + \"&current=temperature_2m,weather_code&timezone=auto\";\n    const repMeteo = await fetch(urlMeteo);\n    const meteo = await repMeteo.json();\n\n    const urlDrapeau = \"https://flagcdn.com/\" + res.country_code.toLowerCase() + \".svg\";\n    const carte = document.createElement(\"div\");\n    carte.className = \"carte\";\n    carte.innerHTML = `<h2>${res.name}</h2><img src=\"${urlDrapeau}\" alt=\"\" width=\"40\"><p>${res.country} — ${meteo.current.temperature_2m} °C</p>`;\n    conteneur.append(carte);\n  } catch (e) {\n    alert(\"Erreur réseau : \" + e.message);\n  }\n\n  champ.value = \"\";\n});"
        },
        en: {
          fichier: "script.js",
          intro: "Replace all of <code>script.js</code> with this code using <code>async/await</code>, chained requests, and error handling:",
          code: "const formulaire = document.querySelector(\"#form-ville\");\nconst champ = document.querySelector(\"#champ-ville\");\nconst conteneur = document.querySelector(\"#cartes\");\n\nformulaire.addEventListener(\"submit\", async function(event) {\n  event.preventDefault();\n  const ville = champ.value;\n  if (ville === \"\") return;\n\n  try {\n    const urlGeo = \"https://geocoding-api.open-meteo.com/v1/search?name=\" + ville + \"&count=1&language=fr\";\n    const repGeo = await fetch(urlGeo);\n    const geo = await repGeo.json();\n\n    if (!geo.results) { alert(\"Ville introuvable : \" + ville); return; }\n\n    const res = geo.results[0];\n    const urlMeteo = \"https://api.open-meteo.com/v1/forecast?latitude=\" + res.latitude\n      + \"&longitude=\" + res.longitude\n      + \"&current=temperature_2m,weather_code&timezone=auto\";\n    const repMeteo = await fetch(urlMeteo);\n    const meteo = await repMeteo.json();\n\n    const urlDrapeau = \"https://flagcdn.com/\" + res.country_code.toLowerCase() + \".svg\";\n    const carte = document.createElement(\"div\");\n    carte.className = \"carte\";\n    carte.innerHTML = `<h2>${res.name}</h2><img src=\"${urlDrapeau}\" alt=\"\" width=\"40\"><p>${res.country} — ${meteo.current.temperature_2m} °C</p>`;\n    conteneur.append(carte);\n  } catch (e) {\n    alert(\"Erreur réseau : \" + e.message);\n  }\n\n  champ.value = \"\";\n});"
        }
      },
      quiz: [
