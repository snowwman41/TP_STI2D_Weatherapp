...
export const m0 = {
  ...
  etapes: [
    ...
    },
    {
      ...
      decouverte: {
        fr: "<strong>L'éditeur en ligne</strong> te permet d'apprendre et d'expérimenter chaque nouvelle notion avec un résultat instantané dans l'aperçu — pas besoin de sauvegarder ni d'ouvrir un navigateur. <strong>Notepad++</strong>, lui, te sert à construire ton <em>vrai projet</em> dans un dossier que tu vas créer toi-même, <code>app_web_meteo/</code> : c'est là que les fichiers <code>index.html</code>, <code>style.css</code> et <code>script.js</code> — que tu vas créer toi-même — prennent vie pour de bon.",
        en: "<strong>The online editor</strong> lets you learn and experiment with each new concept, with instant feedback in the preview — no saving or browser needed. <strong>Notepad++</strong> is where you build your <em>real project</em>, in a folder you create yourself, <code>app_web_meteo/</code>: that is where <code>index.html</code>, <code>style.css</code>, and <code>script.js</code> — files you create yourself — truly come to life."
      },
      explication: {
        fr: "La boucle de travail à retenir est simple : <strong>(1)</strong> tu apprends une notion en ligne dans l'éditeur de la plateforme, tu l'expérimentes ; <strong>(2)</strong> tu reportes ce que tu viens de pratiquer dans tes fichiers Notepad++ (<code>app_web_meteo/</code>) ; <strong>(3)</strong> tu enregistres avec <kbd>Ctrl+S</kbd> ; <strong>(4)</strong> tu ouvres ou rafraîchis <code>index.html</code> dans le navigateur — par double-clic — et tu vois <em>ton</em> projet avancer. Pas « faire tout en ligne puis copier-coller à la fin » : module après module, ton vrai projet grandit en parallèle de ton apprentissage.<br><br>Mais d'abord, tu dois créer toi-même ton dossier de projet et tes fichiers — rien ne t'est donné à l'avance. La section ci-dessous t'explique pas à pas comment faire."
,
        en: "The workflow to remember is simple: <strong>(1)</strong> you learn a concept online in the platform editor, you experiment; <strong>(2)</strong> you transfer what you just practised into your Notepad++ files (<code>app_web_meteo/</code>); <strong>(3)</strong> you save with <kbd>Ctrl+S</kbd>; <strong>(4)</strong> you open or refresh <code>index.html</code> in the browser — by double-clicking — and you watch <em>your</em> project grow. Not « do everything online then copy-paste at the end »: module by module, your real project grows alongside your learning.<br><br>But first, you must create your project folder and files yourself — nothing is handed to you. The section below walks you through it step by step."
      },
      illustration: null,
      exemple: {
        code: "// La boucle en 4 temps :\n// 1. J'apprends la notion ici, dans l'éditeur en ligne\n// 2. Je reporte dans app_web_meteo/ avec Notepad++\n// 3. Ctrl+S pour enregistrer\n// 4. Je rafraîchis index.html dans le navigateur → mon projet avance !",
        langage: "text",
        commentaire: {
      ...
      exercice: null,
      application: {
        fr: "Tu vas créer ton projet de A à Z — rien ne t'est donné à l'avance. Suis ces étapes une seule fois, au tout début :<br><br><strong>1. Crée le dossier du projet.</strong> Ouvre ton <strong>espace personnel</strong> → <strong>Documents</strong>, fais un clic droit dans un espace vide → <strong>Nouveau → Dossier</strong>, et nomme-le exactement <code>app_web_meteo</code> (en minuscules, sans espace).<br><br><strong>2. Ouvre Notepad++.</strong> Menu Démarrer → tape « Notepad++ » → Entrée.<br><br><strong>3. Ouvre ton dossier en tant qu'espace de travail.</strong> Dans Notepad++, va dans le menu <strong>Fichier → Ouvrir le dossier en tant qu'espace de travail…</strong>, puis sélectionne le dossier <code>app_web_meteo</code> que tu viens de créer. Un panneau apparaît à gauche — il est vide pour l'instant, c'est normal.<br><br><strong>4. Crée tes trois fichiers.</strong> Crée un nouveau fichier avec <strong>Fichier → Nouveau</strong> (<kbd>Ctrl+N</kbd>), puis enregistre-le avec <strong>Fichier → Enregistrer sous…</strong> dans le dossier <code>app_web_meteo</code> en le nommant <code>index.html</code>. Recommence pour <code>style.css</code>, puis <code>script.js</code>. Tes trois fichiers (vides pour l'instant) apparaissent maintenant dans le panneau de gauche : tu vas les remplir module après module.<br><br>💡 Astuce : dans le panneau de gauche, un clic droit sur le dossier <code>app_web_meteo</code> propose souvent <strong>Nouveau fichier</strong> — c'est encore plus rapide pour créer tes fichiers.",
        en: "You'll build your project from scratch — nothing is handed to you. Do these steps once, right at the start:<br><br><strong>1. Create the project folder.</strong> Open your <strong>personal space</strong> → <strong>Documents</strong>, right-click in an empty area → <strong>New → Folder</strong>, and name it exactly <code>app_web_meteo</code> (lowercase, no spaces).<br><br><strong>2. Open Notepad++.</strong> Start menu → type « Notepad++ » → Enter.<br><br><strong>3. Open your folder as a workspace.</strong> In Notepad++, go to <strong>File → Open Folder as Workspace…</strong>, then select the <code>app_web_meteo</code> folder you just created. A panel appears on the left — it's empty for now, that's normal.<br><br><strong>4. Create your three files.</strong> Create a new file with <strong>File → New</strong> (<kbd>Ctrl+N</kbd>), then save it with <strong>File → Save As…</strong> inside the <code>app_web_meteo</code> folder, naming it <code>index.html</code>. Repeat for <code>style.css</code>, then <code>script.js</code>. Your three (empty) files now appear in the left panel: you'll fill them in module by module.<br><br>💡 Tip: in the left panel, right-clicking the <code>app_web_meteo</code> folder often offers <strong>New file</strong> — an even faster way to create your files."
      },
      quiz: [
        {
          type: "qcm",
          ...
          options: [
            ...
            { fr: "Dans les fichiers de app_web_meteo/, avec Notepad++", en: "In the app_web_meteo/ files, using Notepad++" },
            { fr: "Dans un nouveau dossier créé à chaque module", en: "In a new folder created for each module" }
          ],
          bonneReponse: 1,
          explication: {
            fr: "L'éditeur en ligne sert à apprendre et tester. C'est Notepad++ et le dossier <code>app_web_meteo/</code> qui contiennent ton vrai projet, celui que tu peux ouvrir dans le navigateur.",
            en: "The online editor is for learning and testing. Notepad++ and the <code>app_web_meteo/</code> folder hold your real project — the one you can open in the browser."
          }
        }
      ],
      scoreMinimal: 1,
